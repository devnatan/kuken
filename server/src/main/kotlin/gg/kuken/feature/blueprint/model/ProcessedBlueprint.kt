...
data class ProcessedBlueprint(
    ...
    @Serializable
    data class Descriptor(val name: String, val version: String)
        ...
        data class Docker(
            ...
            class DynamicPropertySerializer : KSerializer<Property> {
                ...
                override val descriptor: SerialDescriptor = buildSerialDescriptor(
                    serialName = "gg.kuken.feature.blueprint.model.ProcessedBlueprint.Property",
                ...
                override fun serialize(encoder: Encoder, value: Property) {
                    when (value) {
                        ...
                        Property.Unresolved -> Unit
                    }
                }

                override fun deserialize(decoder: Decoder): Property = try {
                ...
                private fun deserializeAsStructure(decoder: Decoder): Property = decoder.decodeStructure(descriptor) {
                    lateinit var property: Property
                    lateinit var dependencies: List<Property>
                    ...
                    while (true) {
                        ...
                        if (field == "property")
                            property = decodeSerializableElement(
                                descriptor = descriptor,
                                index = index + 1,
                                deserializer = Property.Constant.serializer()
                            )

                        if (field == "literal") literal = decodeStringElement(descriptor, index + 1)

                        if (field == "dependencies")
                            dependencies = decodeSerializableElement(
                                descriptor = descriptor,
                                index = index + 1,
                                deserializer = ListSerializer(Property.serializer())
                            )
                    }

                    Property.Dynamic(literal, property, dependencies)
                }
            }
        }
    }
    ...
    sealed class Property {
        ...
        @Serializable
        @SerialName("constant")
        data class Constant(val value: String) : Property()

        @Serializable
        @SerialName("input")
        data class Input(val name: String) : Property()

        @Serializable
        @SerialName("env")
        data class EnvironmentVariable(val env: String) : Property()

        @Serializable
        @SerialName("placeholder")
        data class Placeholder(val expr: String) : Property() {
        ...
        data class Dynamic(
            ...
            class Serializer : KSerializer<Dynamic> {
                ...
                override fun serialize(encoder: Encoder, value: Dynamic) = encoder.encodeStructure(descriptor) {
                ...
                override fun deserialize(decoder: Decoder): Dynamic = decoder.decodeStructure(descriptor) {
